# -*- coding: utf-8 -*-
"""
眼动 PC1-PC5 与六个量表总分的 Spearman 相关分析。

分析流程：
1. 读取六个量表 Excel，提取每名被试的原始总分。
2. 读取眼动 PCA component scores，提取 PC1-PC5。
3. 按手机号优先、唯一姓名其次的规则匹配被试。
4. 对每个“PC × 量表总分”组合执行 Spearman 秩相关。
5. 对全部相关检验执行 Benjamini-Hochberg FDR 校正。
6. 输出一个包含完整结果和匹配审计信息的 Excel 工作簿。

说明：
- 原始 Excel schema 保持中文，不修改源文件数据结构。
- 六个量表均使用原始总分。
- 不对量表总分做 PCA、z 标准化或反向转换。
- 注意控制、心力、MMSE：总分越高越好。
- 抑郁、焦虑、失眠：总分越低越好。
"""
...
OUTPUT_XLSX = (
    SCRIPT_DIR
    / "Chapter5_Output2"
    / "eye_PC1_PC5_scale_spearman_results.xlsx"
)
...
def load_pc_file(path: Path) -> pd.DataFrame:
    """读取眼动 PC1-PC5 文件。"""
...
def match_scale_to_pc(
    ...
    resolved = pc.apply(resolve, axis=1)

    pc["person_key"] = resolved.map(lambda item: item[0])
    pc["match_method"] = resolved.map(lambda item: item[1])
...
def interpret_spearman_result(
    ...
    if abs(rho) < 1e-12:
        return f"该 PC 与{scale_cn}总分几乎无单调相关"

    if direction == "higher_is_better":
        if rho > 0:
            return f"该 PC 越高，{scale_cn}总分越高，通常表示状态越好"
        else:
            return f"该 PC 越高，{scale_cn}总分越低，通常表示状态越差"

    if direction == "lower_is_better":
        if rho > 0:
            return f"该 PC 越高，{scale_cn}总分越高，通常表示症状更重 / 状态更差"
        else:
            return f"该 PC 越高，{scale_cn}总分越低，通常表示症状更轻 / 状态更好"

    return ""


def run_spearman_pc_analysis(
    data: pd.DataFrame,
    pc_columns: list[str],
    scale_columns: list[str],
) -> pd.DataFrame:
    """对每个 PC 与每个量表总分做 Spearman 相关。"""
    rows: list[dict[str, Any]] = []

    for pc_column in pc_columns:
        for scale in scale_columns:
            if pc_column not in data.columns or scale not in data.columns:
                continue

            pair = pd.DataFrame({
                "pc_score": safe_numeric(data[pc_column]),
...
def build_output_tables(
    ...
    count_table = pd.DataFrame([
        ...
        {
            "item": "pc_columns_count",
            "value": len(PC_COLUMNS),
        },
        {
    ...
    readme_table = pd.DataFrame({
        ...
        "value": [
            "Eye PC1-PC5 vs questionnaire total score",
            "Eye movement PCA component scores PC1-PC5",
            "Six questionnaire raw total scores",
            "Spearman rank correlation",
            ", ".join(PC_COLUMNS),
            "No PCA, no z-score, no reverse transformation for scale scores",
            "Benjamini-Hochberg FDR across all PC × scale tests",
            MIN_CORRELATION_N,
            (
...
def main() -> None:
    ...
    # 2. 读取眼动 PC1-PC5
    pc_data = load_pc_file(PC_CSV)

    print("本次分析使用的眼动 PC 列：")
    for column in PC_COLUMNS:
        print(f"  - {column}")

    # 3. 匹配量表与 PC 数据
    merged, match_audit = match_scale_to_pc(
    ...
    # 4. Spearman 相关分析
    spearman_results = run_spearman_pc_analysis(
        data=analysis_data,
